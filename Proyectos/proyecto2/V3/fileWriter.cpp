...
void *FileWriter::start(void *ptr)
{
    ...
    while (1)
    {
        ...
        // f = fopen("stats.csv", "a");
        // fprintf(f, "%d,%d,%d,%d,%d,%d,%d,%d,%d\n", inst->vecRegs->vectorRegs[1][2], inst->instMem->instMemory[1], inst->instMem->instMemory[2], inst->instMem->instMemory[3],
        ...
        if(!inst->clk->clockControl){
            ...
            for(int i=0; i<10; i++){
                 std::cout<<"V"<<i<<": ";
                for(int j=0; j<8; j++){
                    std::cout<<inst->vecRegs->vectorRegs[i][j]<<" ";
                }
                std::cout<<"\n";
            }
            std::cout<<"\n**********SCALAR REGISTERS**********"<<std::endl;
            for(int i=0; i<10; i++){
                std::cout<<"S"<<i<<": "<<inst->scaRegs->scalarRegs[i]<<", ";
            }
            std::cout<<"\n\n\n";
        }
         pthread_mutex_unlock(&(inst->clk->clockMutex));
